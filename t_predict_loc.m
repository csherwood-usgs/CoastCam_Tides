function yout=t_predic(tim,nam2,varargin);
...
if longseries
  ...
  for k=1:size(nam2,2)
    %ju(k)=strmatch(nam2{k},const.name);
    ju(k)=find(abs(freq(k)-const.freq)==min(abs(freq(k)-const.freq)));
    %ju(k)=startsWith(nam2{k},const.name);
    %ju(k)=startsWith(const.name,names(k,:));
    %ju(k)=startsWith(const.name,nam2{k});
  end;
...
for j1=1:nsub:ntim
  ...
  yout(j1:j2)=sum(exp( 1i*2*pi*freq*tim(j1:j2)*24).*ap(:,ones(1,j2-j1+1)),1)+ ...
              sum(exp(-1i*2*pi*freq*tim(j1:j2)*24).*am(:,ones(1,j2-j1+1)),1);
end;
